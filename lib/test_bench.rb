  ...
  class Base
    ...
    #class << self
      ...
      def iterate( php, hosts, middleware, scenarios, test_cases )
        ...
        run([final_test_cases], global_db, local_db, 0, 0)
...
#              end # end scn_set
  ...
  class TestBenchRunContext
    attr_reader :semaphore1, :semaphore2, :semaphore3, :semaphore4, :semaphore5, :results, :chunk_replacement
    def initialize
    ...
    def key(host, php, middleware, scenarios)
      
    end
    def get_key(host_or_php_or_middleware_or_scenarios)
      1
    end
    def next_host
      # TODO skip to next host
    end
    def rerun
      # TODO repeat run
    end
